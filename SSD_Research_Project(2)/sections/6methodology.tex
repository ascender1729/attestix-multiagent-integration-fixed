...
\begin{itemize}
    ...
    \item \textbf{Prompt-injection mitigation (OWASP LLM01).} Every external input that flows into an LLM prompt (patient statement, ticker symbol, supplier name, contract requirements, DuckDuckGo search results, internal supplier documents, news feeds) is now wrapped in spotlighting delimiters (\texttt{<<<USER\_CONTENT>>>...<<<END\_USER\_CONTENT>>>}) by a shared helper at \texttt{shared/prompt\_safety.py}. The accompanying system-prompt suffix instructs the model to treat the delimited content strictly as data, never as instructions. A regex layer flags the canonical injection vocabulary (\textit{ignore previous instructions}, \textit{disregard the above}, \textit{reveal your system prompt}, \texttt{<system>} tags, embedded delimiter strings) and annotates the wrapped block with a possible-injection note before forwarding it to the LLM. Three pytest cases in \texttt{tests/test\_attestix\_integration.py} cover the wrap, the detection vocabulary, and idempotence of the system-prompt hardening. This is defense-in-depth; it does not guarantee zero exploitation but it raises the cost of the common attack vectors and documents the safety posture for regulators.
    \item \textbf{Synthetic data and lawful basis.} All four domain databases (\texttt{mock\_hospital\_db.json}, \texttt{quant\_strategies\_db.json}, \texttt{mock\_legal\_db.json}, \texttt{internal\_supplier\_docs.json}, \texttt{mock\_news\_api.json}, \texttt{mock\_market\_api.json}) are synthetic. No real PHI, PII, customer records, trading IP, or licensed legal data is processed by the PoC. Production deployment with real data in any of the four domains would require: (i) a documented lawful basis under DPDP Act 2023 / GDPR (consent, contract, or legitimate interest, depending on domain and jurisdiction); (ii) a Data Processing Agreement with the LLM inference provider (Standard Contractual Clauses or an equivalent transfer mechanism for cross-border processing to Groq's US infrastructure); (iii) data-minimisation and pseudonymisation at intake to limit the scope of sensitive-category processing. Each mock data file in the repository carries a \texttt{\_synthetic\_data\_notice} key making the synthetic status machine-readable.
\end{itemize}
...
Privilege escalation (single-hop) & UCAN capability scoping at gate verification & $\checkmark$ Mitigated (PoC scope; multi-hop is Future Work) \\
...
Prompt injection in untrusted inputs & Spotlighting delimiters + injection-pattern detector (OWASP LLM01); 3 pytest cases & $\checkmark$ Partially mitigated (defense-in-depth) \\
\hline
\end{tabularx}
\end{table}
